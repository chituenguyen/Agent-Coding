...
function buildSteps(workspaceName) {
  return [
    {
      icon: (
        <svg
      ...
      description:
        "Describe what you want to build or fix — Claude spawns a team of specialized agents that plan, code, review, and commit automatically.",
      color: "indigo",
    },
    {
      icon: (
      ...
      description:
        "Create a task with a description and optional repo path. Claude runs Architect → Coder → Reviewer → Debugger → Commit automatically. Track progress in real time.",
      color: "violet",
      badge: "/tasks",
    },
    {
      ...
      description:
        "Describe a bug and the Investigator agent traces it to the root cause — file path, line number, and causal chain. Optionally auto-fix and verify with tests.",
      color: "blue",
      badge: "/investigate",
    },
    {
      ...
      description:
        "Add several tasks to a queue and let Claude process them sequentially — overnight, while you're in a meeting, or any time. New tasks can be added while the queue runs.",
      color: "cyan",
      badge: "/queue",
    },
    {
      ...
      description:
        "Add Model Context Protocol servers to give Claude access to GitHub, databases, Slack, browsers, and more. Browse the catalog for one-click installs.",
      color: "emerald",
      badge: "/mcp",
    },
    {
      ...
      description:
        'Each agent has a "soul" — edit their instructions, model, and linked skills to shape how they think and act. No code needed, just plain text.',
      color: "orange",
      badge: "/agents",
    },
  ];
...
const COLORS = {
  indigo: {
    ...
    ring: "bg-indigo-100 dark:bg-indigo-900",
    dot: "bg-indigo-600",
    btn: "bg-indigo-600 hover:bg-indigo-700",
  },
  violet: {
    bg: "bg-violet-50 dark:bg-violet-950",
    ring: "bg-violet-100 dark:bg-violet-900",
    dot: "bg-violet-600",
    btn: "bg-violet-600 hover:bg-violet-700",
  },
  blue: {
    bg: "bg-blue-50 dark:bg-blue-950",
    ring: "bg-blue-100 dark:bg-blue-900",
    dot: "bg-blue-600",
    btn: "bg-blue-600 hover:bg-blue-700",
  },
  cyan: {
    bg: "bg-cyan-50 dark:bg-cyan-950",
    ring: "bg-cyan-100 dark:bg-cyan-900",
    dot: "bg-cyan-600",
    btn: "bg-cyan-600 hover:bg-cyan-700",
  },
  emerald: {
    bg: "bg-emerald-50 dark:bg-emerald-950",
    ring: "bg-emerald-100 dark:bg-emerald-900",
    dot: "bg-emerald-600",
    btn: "bg-emerald-600 hover:bg-emerald-700",
  },
  orange: {
    bg: "bg-orange-50 dark:bg-orange-950",
    ring: "bg-orange-100 dark:bg-orange-900",
    dot: "bg-orange-500",
...
export default function Onboarding() {
  ...
  useEffect(() => {
    ...
    api
      .getWorkspaceName()
      .then((r) => {
        if (!cancelled) setWorkspaceName(r?.name || DEFAULT_WORKSPACE_NAME);
      })
      .catch(() => {
        /* keep default */
      });
  ...
  const STEPS = buildSteps(workspaceName);
    ...
    >
      ...
      >
        ...
        <div className="px-8 pt-8 pb-6 h-72 flex flex-col items-center justify-center">
          {/* Icon */}
          <div
            className={`w-20 h-20 ${c.ring} rounded-2xl flex items-center justify-center mb-5 shrink-0`}
          >
            {s.icon}
          </div>
          ...
          <div className="h-6 flex items-center justify-center mb-1">
            {s.badge && (
              <span
                className={`text-xs font-mono px-2 py-0.5 rounded-full ${c.bg} ${c.dot.replace("bg-", "text-")}`}
              >
                {s.badge}
              </span>
            )}
          ...
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400 text-center mt-0.5">
            {s.subtitle}
          </p>
          <p className="text-sm text-gray-600 dark:text-gray-300 text-center mt-3 leading-relaxed">
            {s.description}
          </p>
        </div>
