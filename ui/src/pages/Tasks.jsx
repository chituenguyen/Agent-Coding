...
export default function Tasks() {
  ...
  async function handleDelete(task, e) {
    e.stopPropagation();
    if (!confirm(`Delete task "${plainDescription(task.description)}"?`))
      return;
    try {
  ...
  async function handleClearDone() {
    ...
    if (
      !confirm(
        `Delete ${done.length} completed task${done.length !== 1 ? "s" : ""}?`,
      )
    )
      return;
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* ── Left: task list ── */}
      <div
        className={`flex flex-col border-r border-gray-200 dark:border-gray-700 overflow-hidden transition-all ${selected ? "hidden md:flex w-80 xl:w-96 shrink-0" : "flex-1 max-w-3xl"}`}
      >
        {/* Header */}
        ...
        {!loading && projects.length > 1 && (
          <div className="flex items-center gap-1.5 px-6 pb-3 flex-wrap shrink-0">
            ...
            >
              All{" "}
              <span
                className={`ml-1 px-1.5 py-0.5 rounded-full text-xs ${activeProject === "all" ? "bg-indigo-500" : "bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400"}`}
              >
                {tasks.length}
              </span>
            </button>
            {projects.map((p) => (
              ...
              >
                {p}{" "}
                <span
                  className={`ml-1 px-1.5 py-0.5 rounded-full text-xs ${activeProject === p ? "bg-indigo-500" : "bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400"}`}
                >
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <p
          className={`text-sm font-medium truncate transition-colors ${selected ? "text-indigo-700 dark:text-indigo-300" : "text-gray-900 dark:text-white group-hover:text-indigo-700 dark:group-hover:text-indigo-300"}`}
        >
...
function TaskDetailPanel({ project, taskId, onClose, onDeleted }) {
  ...
  async function handleResetItem(type, item) {
    ...
    if (
      !confirm(
        `Reset this ${label}? All generated files (SPEC, code, review) will be deleted. Only the original description will be kept.`,
      )
    )
      return;
    try {
