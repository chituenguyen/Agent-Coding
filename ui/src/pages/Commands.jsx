...
export default function Commands() {
  ...
  async function load() {
    setLoading(true)
    try { setCommands(await api.getCommands()) }
  ...
  async function handleSave() {
    ...
    try {
      if (modal.mode === 'create') {
        await api.createCommand({ filename: form.filename.trim(), content: form.content })
      } else {
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Slash commands invoked with /command-name in Claude</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {loading ? (
        <div className="text-gray-400 dark:text-gray-500 text-sm">Loading commands...</div>
      ) : commands.length === 0 ? (
        <div className="text-center py-16 text-gray-400 dark:text-gray-500">
          <p className="font-medium text-gray-600 dark:text-gray-300">No commands defined</p>
          <p className="text-sm mt-1">Commands are slash-invocable instructions for Claude</p>
        ...
        <div className="space-y-2">
          {commands.map((cmd) => (
            <div key={cmd.filename} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex items-start gap-4 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-gray-900 dark:text-white font-mono">/{cmd.filename}</h3>
                </div>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 truncate">{getPreview(cmd.content)}</p>
              </div>
      ...
      {modal && (
        <Modal
          title={modal.mode === 'create' ? 'New Command' : `Edit: /${modal.cmd.filename}`}
        ...
        >
          <div className="space-y-4">
            {saveError && (
              <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 text-red-700 dark:text-red-400 text-sm">{saveError}</div>
            )}
            ...
            {modal.mode === 'create' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1.5">
                  Command Name
                  <span className="text-xs text-gray-400 dark:text-gray-500 font-normal ml-1.5">— invoked as /{'{name}'} in Claude</span>
                </label>
                <div className="flex items-center border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                  <span className="px-3 py-2 bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-sm border-r border-gray-300 dark:border-gray-600 font-mono">/</span>
                  <input
                    value={form.filename}
                    onChange={e => setForm(f => ({ ...f, filename: e.target.value.replace(/[^a-z0-9-]/g, '-') }))}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1.5">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500 font-normal ml-1.5">— defines what Claude does when this command is invoked</span>
              </label>
              <textarea
                rows={16}
                value={form.content}
