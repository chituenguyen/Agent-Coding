...
export default function Queue() {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    try {
      return new Set(
        JSON.parse(localStorage.getItem("queue-collapsed") || "[]"),
      );
    } catch {
      return new Set();
    }
  ...
  function toggleCollapse(key) {
    setCollapsed((prev) => {
      ...
      try {
        localStorage.setItem("queue-collapsed", JSON.stringify([...next]));
      } catch {}
      return next;
    });
  }
  ...
  async function load() {
    try {
      setQueue(await api.getQueue(companyId));
    } catch {
      // ignore
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const hasActive = queue.tasks.some(
  ...
  const completed = tasks
    ...
    .sort((a, b) =>
      (b.finished_at || b.added_at || "").localeCompare(
        a.finished_at || a.added_at || "",
      ),
    );
  ...
  function navTo(task) {
    if (task.type === "fix" || task.type === "subtask") {
      ...
      if (parts?.length >= 3) {
        ...
        const base = companyId ? `/co/${companyId}/tasks` : "/tasks";
        navigate(`${base}?project=${project}&task=${taskId}&tab=${tab}`);
      }
    } else if (task.project && task.task_id) {
      const base = companyId ? `/co/${companyId}/tasks` : "/tasks";
      navigate(
        `${base}?project=${task.project}&task=${task.task_id}&tab=overview`,
      );
    }
  }
  ...
  function getTrackPath(task) {
    return (
      ...
      task.subtask_path ||
      task.task_path ||
      `tasks/${task.project}/${task.task_id}`
    );
  }
        ...
        <div>
          {companyId && (
            <button
              ...
              className="mb-1 inline-flex items-center gap-1 text-[11px] text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white"
            >
              ← {companyId}
            </button>
          )}
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {loading
              ? "..."
              : `${tasks.length} items · ${counts.pending} pending${counts.running ? ` · ${counts.running} running` : ""} · ${counts.done} done${counts.failed ? ` · ${counts.failed} failed` : ""}${companyId ? ` · ${companyId}` : ""}`}
          </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-gray-400 dark:text-gray-500">
          ...
          <p className="text-sm mt-1">
            Add tasks, bug fixes, or sub-tasks from the Tasks page — they'll be
                  ...
                  return (
                    <div key={proj}>
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-400 dark:text-gray-500">
                          {completedByProject[proj].length}
                        </span>
                      </button>
                      {open && (
                        <div className="space-y-1.5">
...
const TYPE_CONFIG = {
  fix: {
    label: "Bug Fix",
    cls: "bg-orange-100 dark:bg-orange-900 text-orange-700 dark:text-orange-300",
  },
  subtask: {
    label: "Sub-task",
    cls: "bg-violet-100 dark:bg-violet-900 text-violet-700 dark:text-violet-300",
  },
  investigate: {
    label: "Investigate",
...
function QueueRow({
  ...
  return (
    <div
      className={`rounded-lg border transition-all bg-white dark:bg-gray-900
      ${
        task.status === "running"
    ...
    >
      ...
      >
        ...
        {showReorder ? (
          <div className="flex flex-col items-center gap-0.5 shrink-0 mt-0.5">
            <button
              ...
              disabled={isFirst}
              className="p-0.5 text-gray-300 dark:text-gray-600 hover:text-indigo-500 dark:hover:text-indigo-400 disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
              title="Move up"
            >
              <svg
                className="w-3.5 h-3.5"
            ...
            <span className="text-xs font-mono text-gray-400 dark:text-gray-500 leading-none">
              {position}
            </span>
            <button
              onClick={(e) => {
          ...
          <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">
            {task.status === "running" ? (
              <svg
                className="w-3.5 h-3.5 text-yellow-600 animate-spin"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            <span
              className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${typeCfg.cls}`}
            >
              {typeCfg.label}
            </span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {task.added_at && (
              <span className="text-xs text-gray-400 dark:text-gray-500">
                {new Date(task.added_at).toLocaleString("en-US", {
                  month: "short",
                  day: "numeric",
                  hour: "2-digit",
                  minute: "2-digit",
            ...
            {task.finished_at && (
              <span className="text-xs text-gray-400 dark:text-gray-500">
                ...
                {new Date(task.finished_at).toLocaleString("en-US", {
                  month: "short",
                  day: "numeric",
                  hour: "2-digit",
                  minute: "2-digit",
          ...
          {task.error && (
            <p className="text-xs text-red-600 dark:text-red-400 mt-1 bg-red-50 dark:bg-red-950 rounded px-2 py-1">
              {task.error}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span
            className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${cfg.cls} ${task.status === "running" ? "animate-pulse" : ""}`}
          >
            <span>{cfg.icon}</span>
            {cfg.label}
          </span>
      ...
      {showActions && (
        <div
          className={`flex items-center gap-2 px-4 pb-3 pt-1 border-t ${
            task.status === "failed"
