...
export default function Mcp() {
  ...
  async function loadRepos() {
    ...
    try {
      setRepos(await api.getRepositories());
    } catch {
      /* ignore */
    } finally {
  ...
  function openAdd(prefill = {}) {
    setModal({
      mode: MODAL_MODE.ADD,
      form: { ...EMPTY_FORM, scope: SCOPE.GLOBAL, ...prefill },
    });
  }
  ...
  async function handleDelete(scope, name) {
    if (
      !(await dialog.confirm({
        message: `Remove "${name}" from ${scope} MCPs?`,
        tone: "danger",
        confirmLabel: "Delete",
      }))
    )
  ...
  async function handleCatalogAdd(item) {
    ...
    if (catalogTarget) {
      ...
      if (hasEmptyEnv) {
        setRepoModal({
          project: catalogTarget,
          mode: MODAL_MODE.ADD,
          form: {
            ...formFromConfig(item.name, SCOPE.REPO, item.config),
            scope: SCOPE.REPO,
          },
        });
      } else {
        ...
        if (expandedRepo === catalogTarget) {
          const d = await api.getRepoMcp(catalogTarget);
          setRepoMcpServers(d.mcpServers || {});
        }
      ...
      if (hasEmptyEnv) {
        setModal({
          mode: MODAL_MODE.ADD,
          form: formFromConfig(item.name, "global", item.config),
        });
      } else {
        await api.upsertMcp(SCOPE.GLOBAL, item.name, item.config);
        load();
  ...
  async function handleBrowseRepoPath() {
    const res = await pickFolder("Select repository folder");
    if (!res) return;
    if (res.remote) {
      setShowRemoteModal(true);
      return;
    }
    applyRepoPath(res.path);
  }
  async function handleAddRepo() {
    ...
    try {
      await api.createRepository({
        path: addRepoForm.path.trim(),
        name: addRepoForm.name.trim() || undefined,
      });
      setAddRepoModal(false);
      setAddRepoForm({ path: "", name: "" });
      await loadRepos();
    } catch (err) {
      toast.error("Failed to add repository: " + err.message);
    } finally {
      setAddRepoSaving(false);
    }
  }
  async function handleDeleteRepo(name) {
    if (
      !(await dialog.confirm({
        message: `Remove repository "${name}"?\n\nThe projects/${name}/ folder (task history, context) will be kept.`,
        tone: "danger",
        confirmLabel: "Delete",
      }))
    )
      return;
    try {
      await api.deleteRepository(name);
      await loadRepos();
    } catch (err) {
  ...
  async function handleAddCatalog() {
    ...
    try {
      const args = catalogForm.args.trim()
        ? catalogForm.args.trim().split(/\s+/)
        : [];
      const env = Object.fromEntries(
        catalogForm.env.filter((e) => e.k).map((e) => [e.k, e.v]),
      );
      const item = {
        ...
        config: {
          type: "stdio",
          command: catalogForm.command.trim(),
          ...(args.length ? { args } : {}),
          ...(Object.keys(env).length ? { env } : {}),
        },
      };
  ...
  async function toggleRepo(name) {
    ...
    try {
      const d = await api.getRepoMcp(name);
      setRepoMcpServers(d.mcpServers || {});
    } catch {
      setRepoMcpServers({});
    }
  ...
  function openRepoAdd(project) {
    setRepoModal({
      project,
      mode: MODAL_MODE.ADD,
      form: { ...EMPTY_FORM, scope: SCOPE.REPO },
    });
  }
  function openRepoEdit(project, name, cfg) {
    setRepoModal({
      ...
      form: { ...formFromConfig(name, SCOPE.REPO, cfg), scope: SCOPE.REPO },
    });
  }
  async function handleRepoSave() {
    ...
    try {
      ...
      const d = await api.getRepoMcp(project);
      setRepoMcpServers(d.mcpServers || {});
      loadRepos();
      setRepoModal(null);
    } catch (err) {
  ...
  const tabs = [
    { key: TAB.REPOS, label: "Repositories", count: repoCount },
    {
      key: TAB.GLOBAL,
      label: "Global",
      count: Object.keys(data.global).length,
    },
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Manage Model Context Protocol servers — workspace, global, and
            per-repository
          </p>
        </div>
        {activeTab === TAB.REPOS && (
        ...
        {tabs.map((tab) => (
          <button
            ...
            onClick={() => {
              ...
              if (tab.key !== TAB.CATALOG) setCatalogTarget(null);
            }}
          ...
          >
            ...
            {tab.count !== null && (
              <span
                className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? "bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}
              >
                {tab.count}
              </span>
            )}
      ...
      {activeTab === TAB.CATALOG ? (
        ...
        reposLoading ? (
          ...
          <div className="text-center py-16 text-gray-400 dark:text-gray-500">
            ...
            <p className="text-sm mt-1">
              Create a task with a target repo to add one
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3.5">
                  ...
                  >
                    ...
                    <svg
                      className={`w-4 h-4 text-gray-400 transition-transform shrink-0 ${expandedRepo === repo.name ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  {repo.repoPath && (
                    <button
                      onClick={() => setGraphProject(repo.name)}
                  ...
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-red-500 rounded-md hover:bg-red-50 dark:hover:bg-red-950 transition-colors shrink-0"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
                  ...
                  <div className="border-t border-gray-100 dark:border-gray-800 px-4 py-4">
                    {Object.keys(repoMcpServers).length === 0 ? (
                      <div className="text-center py-6 text-gray-400 dark:text-gray-500">
                        <p className="text-sm">
                          No MCP servers for this repository
                        </p>
                        <button
                          onClick={() => {
                            setCatalogTarget(repo.name);
                    ...
                    <div className="mt-4 p-2.5 bg-gray-50 dark:bg-gray-800 rounded-lg text-xs text-gray-400 dark:text-gray-500">
                      ...
                      <code className="bg-gray-200 dark:bg-gray-700 px-1 rounded">
                        projects/{repo.name}/mcp.json
                      </code>{" "}
                      — auto-loaded when workflows run for this repo.
                    </div>
                  </div>
                )}
              </div>
        ...
        loading ? (
          ...
          <div className="text-center py-16 text-gray-400 dark:text-gray-500">
            ...
            <p className="text-sm mt-1">
              Global servers are available in all Claude sessions
            </p>
            <div className="flex items-center gap-2 justify-center mt-4">
              ...
              <button
                ...
                className="px-4 py-2 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-indigo-300 hover:text-indigo-600 text-sm font-medium rounded-lg transition-colors"
              >
                Browse Catalog
              </button>
            </div>
        ...
        >
          <div className="space-y-4">
            <div className="flex gap-3">
              <div className="w-20">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCatalogForm((f) => ({ ...f, icon: e.target.value }))
                  }
                  placeholder="🔌"
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCatalogForm((f) => ({ ...f, label: e.target.value }))
                  }
                  placeholder="My Server"
            ...
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCatalogForm((f) => ({ ...f, category: e.target.value }))
                  }
            ...
            <div>
              ...
              <input
                value={catalogForm.command}
                onChange={(e) =>
                  setCatalogForm((f) => ({ ...f, command: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setCatalogForm((f) => ({ ...f, args: e.target.value }))
                }
            ...
            <div>
              ...
              <KVEditor
                rows={catalogForm.env}
                onChange={(env) => setCatalogForm((f) => ({ ...f, env }))}
      ...
      {repoModal && (
        <McpModal
          ...
          saving={repoSaving}
          onChange={(patch) =>
            setRepoModal((m) => ({ ...m, form: { ...m.form, ...patch } }))
          }
      ...
      {addRepoModal && (
        <Modal
          ...
          onClose={() => setAddRepoModal(false)}
          footer={
            <>
              <button
                onClick={() => setAddRepoModal(false)}
                className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={addRepoSaving || !addRepoForm.path.trim()}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setAddRepoForm((f) => ({ ...f, path: e.target.value }))
                  }
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1">
                Name{" "}
                <span className="font-normal text-gray-400">
                  (optional — auto-derived from folder name)
                </span>
              </label>
              <input
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3">
        <div
          className={`w-2 h-2 rounded-full shrink-0 ${isHttp ? "bg-blue-400" : "bg-green-400"}`}
        />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-gray-900 dark:text-white">
            ...
            <span
              className={`text-xs px-1.5 py-0.5 rounded font-mono ${isHttp ? "bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400" : "bg-green-50 dark:bg-green-950 text-green-600 dark:text-green-400"}`}
            >
